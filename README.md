...
結合「連續滾動流暢感」與「節點展開結構感」的雙欄直式編年表網頁應用，從人類文明的曙光（約西元前 15000 年長濱文化）到當代，涵蓋台灣、中國、日本、世界四條並列時間線，可依地區篩選與時間排序方向切換，事件說明點擊卡片展開。共 2200+ 筆事件，每筆皆附維基百科驗證連結。前端完全靜態化、資料本地化——所有歷史事件由 `src/data/events.json` 驅動，不呼叫任何 LLM API，杜絕 API Key 暴露風險與流量費用。
