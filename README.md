# 台灣百年重大事件編年表 (1926 - 2026)

結合「連續滾動流暢感」與「節點展開結構感」的雙欄直式編年表網頁應用。前端完全靜態化、資料本地化——所有歷史事件由 `src/data/events.json` 驅動，不呼叫任何 LLM API，杜絕 API Key 暴露風險與流量費用。
...
時間流向由舊到新：最上方為 1926 年，向下滾動推演至 2026 年；上限年份隨資料自動延伸，無需手動修改。
...
2. `events` 陣列必須維持**由舊到新**排序。
3. `id` 格式為 `evt_YYYYMMDD_序號`；僅知月份、不確定日期時 `day` 填 `null`（id 中日期以 `00` 代替），不可捏造日期。
4. 新增事件後同步更新 `timeline_meta.last_updated`。
